#!/usr/bin/env python3
"""Plot issue #402 30k checkpoint AUPRC against exact-step validation loss."""
...
STEPS = (*range(1_000, 30_000, 1_000), 29_999)
LOSS_KEY = "eval/datasets/dna-exp402-rag-tokenized/loss"
MODELS = {
    "46M": {
        "wandb": "gonzalobenegas/marin/dna-exp402-rag-h640-p46M-30K-scratch",
        "metrics": (
            "gs://marin-us-east5/evals/dna-exp402-rag-h640-p46m-30k/2026.07.26"
        ),
    },
    "104M": {
        "wandb": "gonzalobenegas/marin/dna-exp402-rag-h768-p104M-30K-scratch",
        "metrics": (
            "gs://marin-us-east5/evals/dna-exp402-rag-h768-p104m-30k/2026.07.26"
        ),
    },
}
BENCHMARK_TITLES = {
...
ANNOTATED_STEPS = {1_000, 3_000, 6_000, 12_000, 18_000, 24_000, 29_999}
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("plots/output/issue402_rag_30k_loss_auprc"),
    )
...
def load_validation_losses(api: wandb.Api) -> pl.DataFrame:
    """Fetch exact validation losses at every exported checkpoint."""
...
def load_auprc() -> pl.DataFrame:
    """Load frozen offline metrics at all matching 30k checkpoints."""
    rows: list[dict[str, object]] = []
    for model, sources in MODELS.items():
...
def plot_joined(joined: pl.DataFrame, output_dir: Path) -> None:
    ...
    for benchmark, axis in grid.axes_dict.items():
        ...
        for _, row in subset.iterrows():
            if int(row["step"]) not in ANNOTATED_STEPS:
                continue
            axis.annotate(
    ...
    grid.figure.suptitle("30k ortholog-RAG AUPRC versus exact-step validation loss")
    grid.figure.supxlabel("Validation loss", y=0.075)
    grid.figure.text(
        0.5,
        0.015,
        "Loss decreases left-to-right (x-axis reversed); labels show selected "
        "training steps. Each benchmark y-axis is independently scaled.",
        ha="center",
        fontsize=10,
    )
