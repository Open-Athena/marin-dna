#!/usr/bin/env python3
"""Compare issue #402's final probes and likelihood scores by model size."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--input-46m", default=DEFAULT_INPUTS["46M"])
    parser.add_argument("--input-104m", default=DEFAULT_INPUTS["104M"])
    parser.add_argument(
        "--output-dir",
        type=Path,
...
def load_comparison(input_paths: dict[str, str]) -> pl.DataFrame:
    """Load the preregistered, adequately powered subsets and assert invariants."""
    assert set(input_paths) == set(DEFAULT_INPUTS)
    chunks: list[pl.DataFrame] = []
    for model_size, input_path in input_paths.items():
        ...
        chunks.append(eligible.with_columns(pl.lit(model_size).alias("model_size")))
    comparison = pl.concat(chunks).with_columns(
        pl.col("score_type").replace_strict(SCORE_LABELS).alias("score"),
        pl.col("subset").replace_strict(SUBSET_LABELS).alias("display_subset"),
    )
    assert comparison.group_by("model_size").len()["len"].to_list() == [16, 16]
    return comparison
...
def plot_comparison(comparison: pl.DataFrame, output_dir: Path) -> None:
    ...
    for model_size, axis in grid.axes_dict.items():
        ...
        for score, color in SCORE_COLORS.items():
            score_rows = model_rows[model_rows["score"] == score]
            axis.errorbar(
                score_rows["value"],
    ...
    grid.set_titles("{col_name} parameters")
    grid.figure.suptitle("Ortholog-RAG Mendelian representation probes")
    grid.figure.text(
        0.5,
        0.015,
