...
Vendored **verbatim** from ``eric-czech/marin-dna-post-202606`` @
``2abef91b37a16fde9c9cdf1cfa0046942442b97f`` (``src/figures/mixture_lineage.py``),
alongside the style modules in ``_style/``. Stdlib-only (``dataclasses``), so it
ports unchanged. It carries the training-intrinsic lineage graph + token accounting
(mixture weights, warm-start fork fractions, cumulative-token math) that Figures
9/10 need — this is run metadata, NOT eval results, so it is reused as-is for the
offline redo; only the per-checkpoint AUPRC *source* changes (W&B history →
evals_v2 S3, see ``_mixture.py``). Re-fetch/re-pin per ``_style/README.md``.
