"""Analyze held-out chr21 variant-consequence signals from issue #429."""
...
EXPECTED_DISCOVERY_POSITIVES = 1_024
EXPECTED_EVAL_POSITIVES = 512
CHANCE_LEVEL = 1 / EXPECTED_CLASSES
...
def select_individual_feature(
    ...
    assert discovery_positive.sum() == EXPECTED_DISCOVERY_POSITIVES
    assert validation_positive.sum() == test_positive.sum() == EXPECTED_EVAL_POSITIVES
...
def plot_summary(
    ...
    probe_axis.axvline(
        CHANCE_LEVEL, color="black", linestyle="--", linewidth=1, label="chance"
    )
...
def analyze(
    *,
    ...
    assert panel.height == manifest["panel"]["rows"] == EXPECTED_ROWS
    labels = panel["consequence_cre"].to_numpy()
    split = panel["split"].to_numpy()
