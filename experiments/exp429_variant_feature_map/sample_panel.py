"""Build the frozen balanced chr21 panel for issue 429.
...
EXPECTED_RETAINED_CLASS_COUNT = len(TARGET_CLASSES)
EXPECTED_ROWS = EXPECTED_RETAINED_CLASS_COUNT * sum(QUOTAS.values())
...
def assert_current_commit(value: str) -> None:
    """Require a full commit SHA matching the checked-out experiment code."""
...
def sample_balanced_panel(
    ...
    *,
    ...
    target_classes: tuple[str, ...] | None = TARGET_CLASSES,
    ...
    if target_classes is not None:
        ...
        assert len(set(target_classes)) == len(target_classes)
        frame = frame.filter(pl.col("consequence_cre").is_in(target_classes))
...
def build_panel(
    ...
    assert retained == sorted(TARGET_CLASSES), retained
