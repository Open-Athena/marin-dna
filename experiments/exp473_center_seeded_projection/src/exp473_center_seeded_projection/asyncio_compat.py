...
_PATCH_MARKER = "_exp473_atomic_weakset_snapshot"
...
def _snapshot_weakset(
    ...
    ``set.copy`` is one C operation under the Python 3.12 GIL. Resolving the
    copied weak references afterwards cannot race with registry mutation.
    """
...
def install_all_tasks_snapshot() -> bool:
    ...
    CPython issue 80788 documents that the process-global task weak set can
    change while ``asyncio.run`` shuts down in another thread. Python 3.12's
    bounded retry can still exhaust under TensorStore checkpoint fan-out.
    Later registry implementations do not need this private compatibility
    shim, so unknown layouts are left unchanged.
    """
