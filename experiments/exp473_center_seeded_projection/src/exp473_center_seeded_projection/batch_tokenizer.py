...
"""Case-aware DNA tokenizer copied into the isolated issue #473 project."""
...
class DNABatchTokenizer(BatchProcessor[dict, dict]):
    ...
    def __call__(self, batch: Sequence[dict]) -> list[dict]:
        ...
        assert input_ids.shape == char_weights.shape, (
            f"token count {input_ids.shape[1]} != character count "
            f"{char_weights.shape[1]}; the tokenizer must be character-level"
        )
        ...
        if self._has_eos:
            ...
            loss_weights = np.concatenate(
                [loss_weights, np.ones((batch_size, 1), dtype=np.float32)], axis=1
            )
        return [
