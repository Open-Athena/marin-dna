...
"""DNA format and train-time dispatch vendored into the isolated experiment."""
...
from exp535_4b_uniform_five_region.batch_tokenizer import DNABatchTokenizer
...
def _install_dataset_for_component_patch() -> None:
    ...
    if getattr(datasets.dataset_for_component, "_exp535_dna_patched", False):
        return
    original = datasets.dataset_for_component
    try:
        if "DNALmDatasetFormat" in inspect.getsource(original):
            raise RuntimeError(
                "Levanter now handles DNALmDatasetFormat upstream; remove the exp535 compatibility patch "
                "after verifying its loss-weight key"
            )
    except OSError:
        pass
    ...
    def patched(component, Pos, cache, *, eos_id, block_cross_document_attention):
        if isinstance(component.format, DNALmDatasetFormat):
            return datasets.CausalLmDataset(
                datasets.TokenSeqDataset(cache, Pos.size, loss_weights_key="loss_weight"),
                Pos,
                eos_id=eos_id,
    ...
    patched._exp535_dna_patched = True  # type: ignore[attr-defined]
