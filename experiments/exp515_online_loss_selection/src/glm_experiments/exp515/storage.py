...
def validate_issue_s3_prefix(uri: str) -> tuple[str, str]:
    """Require a versioned issue-515 destination below the prescribed bucket."""
    ...
    if (
        not relative
        or "/" not in relative
        or not relative.rsplit("/", 1)[-1].startswith("v")
    ):
        raise ValueError(
            "checkpoint destination must end in a distinct version directory"
        )
...
def upload_issue_artifact(
    ...
    *,
    ...
    if (
        ...
        raise ValueError(
            "relative S3 artifact path must be nonempty and stay below the prefix"
        )
...
def download_verified_issue_object(
    *,
    ...
    if not key.startswith(ISSUE_PREFIX) or not key.removeprefix(ISSUE_PREFIX):
        raise ValueError(
            f"retained object must stay below s3://{ISSUE_BUCKET}/{ISSUE_PREFIX}"
        )
