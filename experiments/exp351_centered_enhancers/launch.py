...
"""Enhancer-CENTERED vs tiled-control Qwen3-0.25B gLMs (issue #351).

Tests whether defining enhancer training windows by CENTERING on each cCRE (vs
uniform genome tiling) changes distal VEP — a follow-up to exp326 (#326), which
showed curation unsticks distal. Two arms, identical in every respect (dELS+pELS
population, conservation ≥0.20, noexon, 19 one-per-order species, Qwen3-0.25B
geometry, optimizer, 5K steps × 8192 batch, the 5 val_* recipes incl.
``val_enhancer``, mendelian AUPRC eval, ``v5p-8`` hardware) EXCEPT the one axis
under test — window **anchoring**:

Two arms (``SWEEP_DATASETS`` selects which run in a given iris job):
  * ``tiled`` — the tiled control: exp326's curated ``v4_ccre_noexon_enhancer``
    at the one-per-order cohort (uniform 255/128 tiles overlapping the enhancer).
  * ``centered`` — one 255 bp window centered directly on each dELS+pELS cCRE
    (``make_enhancer_anchors``, keep-all), natively projected to the order cohort.

Prediction (P1): ``centered`` distal AUPRC > ``tiled`` (denser enhancer signal
per window); expected small, possibly null. Only the two per-order arms are
compared (no cross-experiment comparison). ⚠️ Size/epoch asymmetry: tiling emits
~2.6× more windows/enhancer, so at fixed 5K×8192 ``tiled`` ≈ 3.7 epochs vs
``centered`` ≈ 9.7 — in-design (compute held fixed, not data); flag at readout.

Dataset build: ``snakemake/zoonomia_projection_dataset`` — the tiled control
reuses exp326's ``v4_ccre_noexon_enhancer`` subset (``species_subset_datasets``,
order cohort); the centered arm is ``workflow/rules/centered.smk``
(``make_enhancer_anchors`` + ``select_anchors_noexon``, own scoped halLiftover).
Both live on HF as
``bolinas-dna/zoonomia-v1-{v4_ccre_noexon_enhancer,v4_ccre_enhancer_centered}-order``.
...
# The two exp351 (#351) arms — identical population/curation/species/geometry,
# differing ONLY in window anchoring (the experiment variable). SWEEP_DATASETS
# selects which to launch. Both live on HF at the one-per-order cohort:
#   * tiled    — exp326's curated v4_ccre_noexon_enhancer, order cohort (control).
#   * centered — enhancer-centered windows (snakemake .../rules/centered.smk).
TRAIN_DATASETS: dict[str, str] = {
    "tiled": "bolinas-dna/zoonomia-v1-v4_ccre_noexon_enhancer-order",
    "centered": "bolinas-dna/zoonomia-v1-v4_ccre_enhancer_centered-order",
}
