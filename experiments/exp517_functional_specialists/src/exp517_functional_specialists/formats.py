...
def _install_dataset_for_component_patch() -> None:
    ...
    if getattr(datasets.dataset_for_component, "_exp517_dna_patched", False):
        return
    ...
    try:
        if "DNALmDatasetFormat" in inspect.getsource(original):
            raise RuntimeError(
                ...
                "exp517 compatibility patch after verifying its loss-weight key"
            )
    except OSError:
        pass
    ...
    patched._exp517_dna_patched = True  # type: ignore[attr-defined]
