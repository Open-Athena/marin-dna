...
def test_average_precision_matches_sklearn_with_large_ties() -> None:
    ...
    scores = np.array(
        [[0, 1], [0, 1], [1, 0], [1, 0], [0, 0], [0, 0]], dtype=np.float32
    )
...
def test_analyze_target_excludes_rare_response_and_finds_signal(monkeypatch) -> None:
    rows, features = 100, 4
    ...
    response[:, 3] = 1
    target = Target("overall", "overall", np.arange(rows), labels, True)
    ...
    assert signal["mean_difference"] > 1 and signal["best_auprc"] > 0.9


def test_correct_family_excludes_descriptive_targets_from_bh() -> None:
    base = pl.DataFrame(
        {
            "inferential": [True, True],
            "welch_p": [0.01, 0.04],
            "mann_whitney_p": [0.02, 0.05],
        }
    )
    descriptive = pl.DataFrame(
        {"inferential": [False], "welch_p": [1e-12], "mann_whitney_p": [1e-12]}
    )
    observed = correct_family([base, descriptive])
    np.testing.assert_allclose(observed["welch_q"][:2].to_numpy(), [0.02, 0.04])
    assert np.isnan(observed["minimum_q"][2])


def test_load_dense_pair_reads_projected_columns(tmp_path, monkeypatch) -> None:
