...
from kmer_conservation.core import (INVALID, canonical_codes, exact_index, exact_scores,
                                    make_windows, rank_loci, window_starts)
from kmer_conservation.fixture import assign_components


def row(seq, name="a", start=0, species="human"):
    return dict(sequence=seq, id=name, group=name, component=name, chrom="chr1", start=start,
                end=start+len(seq), species=species, kind="anchor")
...
def test_exact_sparse_matches_bruteforce_including_unseen_query_kmers():
    target = make_windows([row("ACGTACGTAAAACCCC", "a"), row("GGGGTTTTAGCTAGCA", "b")], 8, 3, 2)
    ...
    expected = [max((len(set(a)&set(b))/len(set(a)|set(b)) if len(set(a)|set(b)) else 0)
                    for a in query.features) for b in target.features]
    assert np.allclose(scores, expected)
    assert count == sum(bool(set(a)&set(b)) for a in query.features for b in target.features)
    assert work == sum(len(set(a)&set(b)) for a in query.features for b in target.features)


def test_empty_sets_do_not_match_and_loci_collapse_before_budget():
    target = make_windows([row("AAAAAAAAAAAA", "a"), row("AAAAAAAAAAAA", "b", 100)], 6, 3, 2)
    scores = np.ones(len(target.features))
    assert len(rank_loci(scores, target, limit=10)) == 2
    index, vocab, lengths = exact_index(target)
    observed, count, work = exact_scores([np.array([], dtype=np.uint64)], index, vocab, lengths)
    assert not observed.any() and count == work == 0


def test_overlap_in_either_species_unions_homologs_before_split():
    records = [row("A"*10, "a", 0), row("A"*10, "b", 9),
               row("A"*10, "a", 500, "mouse"), row("A"*10, "b", 1000, "mouse"),
               row("A"*10, "c", 19)]
    assign_components(records, 568, .4)
