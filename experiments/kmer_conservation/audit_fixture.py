...
report = {'component_splits': dict(Counter(split for split, component in set((r['split'],r['component']) for r in anchors))),
          'record_kinds': dict(Counter(r['kind'] for r in rows)), 'central255_matches': {}}
for label in ['human', 'mouse', 'armadillo']:
    sub = [r for r in anchors if r['species'] == label]
    names = [r['group'] for r in sub]
    projected = {r['query_name']: r for r in pl.read_parquet(root/f'{label}.parquet').filter(pl.col('query_name').is_in(names)).to_dicts()}
    exact = 0
    for row in sub:
...
assert not set(r['component'] for r in anchors if r['split']=='dev') & set(r['component'] for r in anchors if r['split']=='heldout')
assert len({(r['species'],r['chrom'],r['start'],r['end']) for r in rows if r['kind'] in ['background','matched_background','repeat_challenge']}) == 3960
