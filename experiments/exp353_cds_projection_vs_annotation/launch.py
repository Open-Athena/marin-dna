...
"""CDS annotation-vs-projection Qwen3-0.25B sweep (issue #353).

Four arms, identical in every respect (Qwen3-0.25B geometry, optimizer, 5K steps
× 8192 batch, the 5 val_* LL-gap recipes, mendelian AUPRC eval) EXCEPT the one
axis under test — the **training dataset** — crossing 2 methods × 2 species
scopes (``SWEEP_DATASETS`` selects which run in a given iris job):
  * ``annot_all204`` / ``annot_vert125`` — Arm A: each species' own annotated v5
    CDS (``genomes-v5`` recipe v5), over all 204 animal orders / the 125-Chordata
    subset.
  * ``proj_all204`` / ``proj_vert125`` — Arm B: human v5 CDS projected onto each
    species via mmseqs2 (``animals-cds-proj-v1``), same two scopes.

Question: does projection ≈ annotation for CDS? The reach diagnostic (#353)
showed nucleotide projection is vertebrate-scale (Chordata ~36% mean window
retention, invertebrates ~2% floor), so the sweep compares annotation vs
projection where both have data (vertebrates) and at animal scale, and isolates
whether the ~3.7% invertebrate projection data helps or hurts.

Datasets (built this session): Arm A is a re-merge of the existing per-genome v5
CDS parquets; Arm B is ``snakemake/.../workflow/rules/cds_projection.smk``. All
four live on HF (see ``TRAIN_DATASETS``).
...
This is a self-contained marin-consumer experiment dir (its own pyproject +
uv.lock, marin in BASE deps — issue #328). Launch from THIS directory, with an
iris tunnel open (see ``experiments/README.md``). Note: NO ``--extra marin`` —
marin is in base deps now, so the coordinator's plain ``uv sync`` already gets
it (and ``--extra marin`` would error: that extra no longer exists).

    cd experiments/exp351_centered_enhancers
    SWEEP_DATASETS=centered uv run iris --cluster=marin job run \\
        --no-wait --user gonzalo --job-name exp351-centered \\
...
# The four exp353 (#353) arms — identical geometry/optimizer/steps/eval,
# differing ONLY in the training dataset (the experiment variable). SWEEP_DATASETS
# selects which to launch. 2 methods × 2 species scopes:
#   * annot_*  — each species' own annotated v5 CDS (Arm A).
#   * proj_*   — human v5 CDS projected via mmseqs2 (Arm B).
#   * *_all204 — all 204 animal orders;  *_vert125 — the 125-Chordata subset.
TRAIN_DATASETS: dict[str, str] = {
    "annot_all204": "bolinas-dna/genomes-v5-genome_set-animals_order204-intervals-v5_255_128",
    "annot_vert125": "bolinas-dna/genomes-v5-genome_set-vertebrates_order125-intervals-v5_255_128",
    "proj_all204": "bolinas-dna/animals-cds-proj-v1-all204",
    "proj_vert125": "bolinas-dna/animals-cds-proj-v1-vert125",
}
...
# Training masks lowercase positions to 1% loss weight. Both arms' datasets
# (genomes-v5 Arm A, animals-cds-proj Arm B) use ``seq`` as the text field.
TRAIN_FORMAT = DNALmDatasetFormat(text_key="seq", lowercase_weight=0.01)
...
def _tokenize(
    ...
    return ExecutorStep(
        ...
        fn=remote(
            ...
            # marin is in BASE deps in this self-contained experiment (#328), so
            # the worker's plain `uv sync` already installs marin + zephyr + jax;
            # no `--extra` needed (an `--extra marin` would error — it no longer
            # exists). `[]` = base only.
...
def _build_data_mixture(strategy: str, dataset: str) -> LmDataConfig:
    ...
    components: dict[str, ExecutorStep] = {
        strategy: _tokenize(
            f"marin_dna-cds353-{strategy}-char-bos", dataset, TRAIN_FORMAT
        ),
    }
...
def _build_train_step(strategy: str, dataset: str) -> ExecutorStep:
    ...
    run_name = f"dna-exp{EXP_ISSUE}-cds-0p25b-{strategy}-{VERSION}"
    ...
    return ExecutorStep(
        ...
        fn=remote(
            ...
            resources=ResourceConfig.with_tpu(TPU_TYPES, ram="300g"),
            # marin in base deps now (#328); only the `tpu` extra adds libtpu.
            pip_dependency_groups=["tpu"],
