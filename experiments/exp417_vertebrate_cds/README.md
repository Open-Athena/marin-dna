...
The source datasets are semantic (repeat-masked), not conservation-filtered.
Uppercase bases receive loss weight 1.0 and lowercase repeat-masked bases
receive loss weight 0.01. The same case-aware format is applied to the
Hugging Face `train` and `validation` splits; the tokenizer lowercases token
identities only after the loss-weight array has been derived from source case.
