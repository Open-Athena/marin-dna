...
def keys(sequence: str, k: int, bits: int) -> list[tuple[int, int]]:
    ...
    for end in range(k, len(sequence) + 1):
        ...
        code = min(
            int("".join(str("ACGT".index(c)) for c in x), 4) for x in [word, reverse]
        )
        key = mix(code ^ 577)
        if key % 2**bits == 0:
...
def test_species_counts_and_unary_scores(
    executable: Path, tmp_path: Path, k: int, bits: int
) -> None:
    ...
    receipt = json.loads(
        subprocess.check_output(
            [str(executable), "build", str(k), str(bits), str(species_list), str(index)]
        )
    )
    observed = {
        key: (count, copies)
        for key, count, copies, _ in struct.iter_unpack("<QIHH", index.read_bytes())
    }
    expected = {
        key: (sum(key in r for r in reference), max(r[key] for r in reference))
    ...
    for row in rows:
        ...
        selected = {
            key for position, key in keys(sequence, k, bits) if start < position <= end
        }
        ...
        for field, function in {
            ...
            assert float(row[field + "_copy4"]) == pytest.approx(
                expected_filtered, abs=1e-6
            )
...
def test_reverse_complement_and_ambiguity(executable: Path, tmp_path: Path) -> None:
    ...
    subprocess.run(
        [str(executable), "build", "9", "0", str(paths), str(index)], check=True
    )
...
def test_batch_scoring_and_input_limit(executable: Path, tmp_path: Path) -> None:
    ...
    result = json.loads(
        subprocess.check_output(
            [str(executable), "build", "9", "0", str(listing), str(index), "128"]
        )
    )
...
def test_query_restriction_preserves_global_counts(
    executable: Path, tmp_path: Path
) -> None:
    rng = random.Random(812)
    query_sequence = "".join(rng.choices("ACGT", k=1000))
    background = "".join(rng.choices("ACGT", k=2000))
    query = tmp_path / "query.fa"
    query.write_text(f">query\n{query_sequence}\n")
    paths = []
    for i, seq in enumerate(
        [
            query_sequence + background + query_sequence,
            background + query_sequence[:500],
            background,
        ]
    ):
        path = tmp_path / f"{i}.fa"
        path.write_text(f">full\n{seq}\n")
        paths.append(path)
    listing = tmp_path / "list"
    listing.write_text("\n".join(map(str, paths)) + "\n")
    full, restricted = tmp_path / "full.bin", tmp_path / "restricted.bin"
    subprocess.run(
        [str(executable), "build", "17", "2", str(listing), str(full)], check=True
    )
    subprocess.run(
        [
            str(executable),
    ...
    def records(path: Path) -> dict:
        return {
            key: (species, copies)
            for key, species, copies, _ in struct.iter_unpack(
                "<QIHH", path.read_bytes()
            )
        }

    expected = records(full)
    observed = records(restricted)
    selected = {key for _, key in keys(query_sequence, 17, 2)}
    assert observed == {key: expected[key] for key in selected}
    assert all(copies >= 2 for _, copies in observed.values())
    outputs = []
    for i, index in enumerate([full, restricted]):
        out = tmp_path / f"scores{i}"
        subprocess.run(
            [
                str(executable),
                "score",
