...
def get_regions(regions, seqlen, regions_used=None):
    ...
    if regions_used is None:
        ...
        regions = [
            ...
            for x in np.array(regions.values)[regions_used]
        ]
...
def save_predictions(output, regions, chrom_sizes, out_dir="./"):
    ...
    data = softmax(parsed_output["pred_profile"]) * (
        np.expand_dims(np.exp(parsed_output["pred_count"]), axis=1)
    )
...
def compare_with_observed(regions, parsed_output, out_dir="./", tag="all_regions"):
    ...
    metrics_dictionary["counts_metrics"]["peaks_and_nonpeaks"]["spearmanr"] = (
        spearman_cor
    )
    ...
    (
        ...
        parsed_output["true_profile"], softmax(parsed_output["pred_profile"])
    )
    plot_histogram(jsd_pw, jsd_rnd, os.path.join(out_dir, "all_regions_jsd"), "")
    metrics_dictionary["profile_metrics"]["peaks_and_nonpeaks"] = {}
    metrics_dictionary["profile_metrics"]["peaks_and_nonpeaks"]["median_jsd"] = (
        np.nanmedian(jsd_pw)
    )
    metrics_dictionary["profile_metrics"]["peaks_and_nonpeaks"]["median_norm_jsd"] = (
        np.nanmedian(jsd_norm)
    )
    ...
    if "is_peak" in regions.columns:
        ...
        metrics_dictionary["profile_metrics"]["peaks"]["median_norm_jsd"] = (
            np.nanmedian(jsd_norm)
        )
        ...
        metrics_dictionary["profile_metrics"]["nonpeaks"]["median_jsd"] = np.nanmedian(
            jsd_pw
        )
        metrics_dictionary["profile_metrics"]["nonpeaks"]["median_norm_jsd"] = (
            np.nanmedian(jsd_norm)
        )
...
def counts_metrics(
    ...
    # fig=plt.figure()
    ax = density_scatter(labels, preds, xlab=xlab, ylab=ylab, fontsize=fontsize)
    plt.suptitle(
        "count: spearman R="
        + str(round(spearman_cor, 3))
        + "\nPearson R="
        + str(round(pearson_cor, 3))
        + "\nmse="
        + str(round(mse, 3)),
        y=0.9,
        fontsize=20,
...
def plot_histogram(region_jsd, shuffled_labels_jsd, output_prefix, title):
    ...
    n, bins, patches = plt.hist(
        region_jsd, num_bins, facecolor="blue", alpha=0.5, label="Predicted vs Labels"
    )
    n1, bins1, patches1 = plt.hist(
        ...
        facecolor="black",
        alpha=0.5,
        label="Shuffled Labels vs Labels",
    )
    plt.xlabel(
        "Jensen Shannon Distance Profile Labels and Predictions in Probability Space"
    )
