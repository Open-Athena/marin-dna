...
class DataConfig:
    ...
    def __init__(
        ...
        test_chroms: List = ["chr5", "chr10", "chr14", "chr18", "chr20", "chr22"],
        exclude_chroms: list = [],
        fold: int = 0,
        genome: str = "hg38",
        batch_size: int = 64,
        ...
        **kwargs,
        ...
        self.chrom_sizes = (
            chrom_sizes if chrom_sizes is not None else _genome.chrom_sizes
        )
        ...
        self.training_chroms = (
            splits_dict["train"] if training_chroms is None else training_chroms
        )
        self.validation_chroms = (
            splits_dict["valid"] if validation_chroms is None else validation_chroms
        )
    ...
    def _validate_chromosomes(self):
        ...
        all_chroms = set(
            self.training_chroms + self.validation_chroms + self.test_chroms
        )
        excluded = set(self.exclude_chroms)

        if not all_chroms:
            raise ValueError(
                "No chromosomes specified for training, validation, or testing"
            )

        if excluded.intersection(all_chroms) != excluded:
            raise ValueError(
                "Some excluded chromosomes are not in the training/validation/test sets"
            )
