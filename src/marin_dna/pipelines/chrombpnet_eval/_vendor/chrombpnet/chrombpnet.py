...
class ArsenalChromBPNet(nn.Module):
    ...
    def __init__(self, config, arsenal_model, **kwargs):
        super().__init__()
        ...
        self.model.iconv = torch.nn.Conv1d(
    ...
    def get_avg_embeddings(self, X, n=4):
        ...
        layers = [
            m
            for m in self.arsenal_model.modules()
            if type(m)
            in [TransformerROPEEncoderLayer, torch.nn.TransformerEncoderLayer]
        ]
        ...
        if self.seq_input_size == self.arsenal_input_size:
            ...
            while right_start < seq_len:
                if right_start + chunk_size <= seq_len:
                    ...
                    curr_probs = self.normalize_probs(
                        self.softmax(curr_logits), curr_tokens
                    )
                    ...
                    end_probs = self.normalize_probs(
                        self.softmax(end_logits), end_tokens
                    )
            ...
            while left_end > 0:
                if left_end - chunk_size >= 0:
                    ...
                    start_probs = self.normalize_probs(
                        self.softmax(start_logits), start_tokens
                    )
        ...
        if self.seq_input_size == self.arsenal_input_size:
            ...
            while right_start < seq_len:
                if right_start + chunk_size <= seq_len:
                    # Full chunk fits
                    curr_tokens = tokens[:, right_start : right_start + chunk_size]
                    curr_embs = self.get_avg_embeddings(
                        curr_tokens, self.num_layers_avg
                    )
            ...
            while left_end > 0:
                if left_end - chunk_size >= 0:
                    ...
                    remainder = left_end
                    start_tokens = tokens[:, :chunk_size]
                    start_embs = self.get_avg_embeddings(
                        start_tokens, self.num_layers_avg
                    )
    ...
    def forward(self, x, **kwargs):
        ...
        if self.arsenal_output_type == "embedding":
            x_embs = self.get_embeddings(
                tokens
            )  # We don't transpose because bpnet code does it for us if dimension is not 4
