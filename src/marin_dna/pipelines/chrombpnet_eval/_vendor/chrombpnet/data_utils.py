...
def one_hot_to_dna(one_hot):
    ...
    assert one_hot.ndim == 3 and one_hot.shape[2] == n_bases, (
        "Expected shape (N, L, 4) or (N, 4)"
    )
...
def format_region(df, width=500):
    df.loc[:, "start"] = (
        df.loc[:, "start"].astype(np.int64) + df.loc[:, "summit"] - width // 2
    )
...
def load_region_df(
    peak_bed, chrom_sizes=None, in_window=2114, shift=0, width=500, is_peak=True
):
    """
    ...
    if chrom_sizes is not None:
        ...
        if df.shape[1] < 10:
            filtered_df = df[
                df.apply(
                    lambda row: (
                        (row.iloc[1] + row.iloc[2]) // 2 - flank_length > 0
                        and (row.iloc[1] + row.iloc[2]) // 2 + flank_length
                        <= int(chrom_sizes.get(row.iloc[0], float("inf")))
                    ),
                    axis=1,
                )
            ]
            ...
            filtered_df = df[
                df.apply(
                    lambda row: (
                        row.iloc[9] + row.iloc[1] - flank_length > 0
                        and row.iloc[9] + row.iloc[1] + flank_length
                        <= int(chrom_sizes.get(row.iloc[0], float("inf")))
                    ),
                    axis=1,
                )
...
def write_bigwig(
    data, regions, gs, bw_out, debug_chr=None, use_tqdm=False, outstats_file=None
):
    ...
    order_of_regs = sorted(
        range(len(regions)), key=lambda x: (chr_to_idx[regions[x][0]], regions[x][1])
    )
