...
Monitor an Iris job continuously and recover on failure. For MarinDNA
training launches, read `.agents/skills/marin-experiment/SKILL.md` first and
preserve its packaging, launch, and completion rules.

## Required Info

1. `job_id` — Iris job ID in canonical format `/<user>/<job>` (e.g., `/dlwh/iris-run-train_tiny_model-20260302-185630`)
2. `iris_target_args` — the exact cluster or config selector used by the launch,
   for example `--cluster=marin`. Preserve the working invocation from
   `marin-experiment`; do not guess an upstream Marin checkout path.
3. `resubmit_command` — the exact original Iris submit command with `--no-wait`.
   For MarinDNA training, do not add upstream tutorial flags such as
   `--extra marin-core:tpu`; preserve the experiment's self-contained
...
- For TPU bad-node errors, report the evidence and proposed intervention to the user.
...
```json
{
  "ts": <timestamp_ms>,
  "job_id": "<JOB_ID>",
  "iris_target_args": "<EXACT_IRIS_TARGET_ARGS>",
...
3a. ON TERMINAL STATE / OOM-LIKE SIGNAL — get a structured per-task summary
   ...
   uv run iris <IRIS_TARGET_ARGS> job summary <JOB_ID>
...
7. RECOVER (STOP -> RESUBMIT)
   - If current job is still non-terminal, stop it first:
     uv run iris <IRIS_TARGET_ARGS> job stop <JOB_ID>
   - Then resubmit:
     <RESUBMIT_COMMAND>
...
- For Zephyr pipeline failures, TPU bad-node errors, or debugging a running
  task with `iris task exec`, report the evidence and proposed intervention to
  the user before taking action beyond this skill's recovery scope.
