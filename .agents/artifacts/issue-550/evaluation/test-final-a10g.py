...
def receipt(model=watch.MODEL):
    return {
        ...
        "files": {
            f"results/{kind}/{model}/{cohort}.{ext}": {
                "bytes": 1,
                "sha256": "a" * 64,
            }
            for kind, ext in [
...
class Contracts(unittest.TestCase):
    ...
    def test_halfway_receipt_cannot_validate_as_final(self):
        model = "dna-exp550-rag46m-five-regions-v1-step-50000"
        watch.validate_receipt(receipt(model), model)
        with self.assertRaises(AssertionError):
            watch.validate_receipt(receipt(model))
        with self.assertRaises(AssertionError):
            watch.validate_receipt(receipt(), model)
    ...
    def test_copy_precedes_stream_and_no_new_ssh_after_completion(self):
        for step in (50000, 100000):
            model = f"dna-exp550-rag46m-five-regions-v1-step-{step}"
            events = []
            worker = {
                "InstanceId": "i-test",
                "Tags": [
                    {
                        "Key": "Name",
                        "Value": f"codex-issue550-vep{step // 1000}k-a10g-bf16",
                    },
                    {"Key": "issue", "Value": "550"},
                ],
            }

            def copy(*args, events=events, **kwargs):
                events.append("copy")

            def stream(*args, events=events, model=model, **kwargs):
                events.append("stream")
                process = MagicMock()
                process.stdout = io.StringIO(
                    "DEVELOPMENT_VEP_OUTPUTS "
                    + json.dumps(receipt(model))
                    + "\nVEP_WORKER_EXIT 0\n"
                )
                process.__enter__.return_value = process
                return process

            def aws(command, events=events, worker=worker, **kwargs):
                action = command[2]
            ...
            with tempfile.TemporaryDirectory() as temporary:
                ...
                with (
                    ...
                    watch.main()
                self.assertTrue(
                    (Path(temporary) / f"step-{step}-completed.json").is_file()
                )
            self.assertEqual(
