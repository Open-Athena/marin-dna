...
def main() -> None:
    ...
    for index in range(3):
        ...
        score(
            model.eval().cuda(),
            tokenizer,
            throughput_data.select(range(index * 32, (index + 1) * 32)),
            batch=best,
            bf16=False,
            compiled=True,
            name=f"separate_partition{index}",
        )
        del model
        gc.collect()
        torch.cuda.empty_cache()
    ...
    score(
        ...
        bf16=False,
        compiled=True,
        name="joint_with_reload",
    )
    joint_seconds = time.monotonic() - start
    report["synthetic_partition_comparison"] = {
        "variants": 96,
        "separate_three_loads_seconds": separate_seconds,
        "joint_one_load_seconds": joint_seconds,
        "saved_seconds": separate_seconds - joint_seconds,
        "note": "Both use an already warmed compiler cache; includes model loading and Trainer setup, not package installation.",
    }
