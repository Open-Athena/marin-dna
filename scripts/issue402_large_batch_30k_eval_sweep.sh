#!/usr/bin/env bash
# Launch bounded offline evaluations for issue #402's 2M-token-batch runs.
#
# Usage:
#   CODE_REVISION=<40-char-sha> scripts/issue402_large_batch_30k_eval_sweep.sh \
#     46m:5000 104m:5000 46m:10000 104m:10000
#
# Allowed cadence: 5k, 10k, ..., 25k, and the final 29,999 HF export.
#
# Environment:
#   MAX_PARALLEL  Maximum simultaneous Sky launch processes (default: 4).
#   LOG_DIR       Local per-cluster logs (default: /tmp/issue402_large_batch_evals).
#   ALLOW_ON_DEMAND
...
wait_batch() {
    ...
    for index in "${!batch_pids[@]}"; do
        ...
        if wait "$pid"; then
            echo "[issue402-large-batch-eval] completed $cluster" >&2
        else
            echo "[issue402-large-batch-eval] FAILED $cluster (see $log_dir/$cluster.log)" >&2
            failures=$((failures + 1))
        fi
    done
    batch_pids=()
...
for spec in "$@"; do
    ...
    step=${BASH_REMATCH[2]}
    if (( (step < 5000 || step > 25000 || step % 5000 != 0) && step != 29999 )); then
        echo "invalid step '$step'; expected 5000, 10000, ..., 25000, or 29999" >&2
        exit 2
    fi

    if [[ "$model" == 46m ]]; then
        artifact="dna-exp402-rag-h640-p46m-b2m-30k"
    else
    ...
    if [[ "$dry_run" == 0 ]]; then
        if ! gcloud storage ls "$checkpoint_uri/config.json" >/dev/null 2>&1; then
            echo "[issue402-large-batch-eval] missing export: $checkpoint_uri/config.json" >&2
            failures=$((failures + 1))
            continue
        fi
        output_complete=1
        ...
        if (( output_complete )); then
            echo "[issue402-large-batch-eval] skip complete $model step $step" >&2
            skipped=$((skipped + 1))
            continue
        fi
    fi
    ...
    if (( dry_run )); then
        printf '[issue402-large-batch-eval] DRY RUN:' >&2
        printf ' %q' "${command[@]}" >&2
        printf '\n' >&2
        continue
    fi

    echo "[issue402-large-batch-eval] launch $model step $step as $cluster" >&2
...
echo "[issue402-large-batch-eval] launched=$launched skipped=$skipped failures=$failures logs=$log_dir" >&2
(( failures == 0 ))
