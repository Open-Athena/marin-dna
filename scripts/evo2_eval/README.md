...
- **(d)** the production `EMB_DTYPE` is adequate for the probe delta — scores once in `float32`, compares the `emb_alt − emb_ref` delta in f32 vs round-tripped through f16 (Pearson), and reports the `|emb|` magnitude distribution. With the float32 default this is informational; with `EMB_DTYPE=float16` it hard-asserts the Pearson. (The first run on the 1B measured Pearson ≈ 0.82 → f16 inadequate, which is why float32 is the default.)
