...
UNIFORM = (
    PREFIX
    + "phylop-uniform-v1/2162b6aa8299a9748eeb8031318b49072bb8c3fc/94d512050de327f96fda1105ce9c6ae5562944e402802516c7cde54795d8cdd1/full/"
)
FUNCTIONAL = (
    PREFIX
    + "functional-v1/e42a4ea1eca760219e0add91004b45cac59b19c9/a104a2756f538a1993405165fb5b50b6d4aeaf0a32810d8bb72907916eef1beb/full/"
)
...
def main() -> None:
    ...
    extras = [
        UNIFORM + "anchors/phylop_uniform_catalog.parquet",
        FUNCTIONAL + "anchors/training.parquet",
    ]
    if not args.anchors_only:
        extras += [
            FUNCTIONAL + "reference/hg38.2bit",
            FUNCTIONAL + "reference/hg38.chrom.sizes",
            ALIAS_KEY,
        ]
    entries = []
    for key in extras:
        ...
        entries.append(
            {"Key": key, "Size": head["ContentLength"], "ETag": head["ETag"]}
        )
    if not args.anchors_only:
    ...
    for entry in entries:
        ...
        started = time.time()
        print(
            json.dumps({"event": "start", "key": key, "bytes": entry["Size"]}),
            flush=True,
        )
        response = client.get_object(
            Bucket="oa-bolinas", Key=key, IfMatch=entry["ETag"]
        )
        ...
        if (
            key == ALIAS_KEY
            and target_digest
            != "56706a5eeaaa368bfcab17bba94437b5a09885fe41494735a42bbb9118bac214"
        ):
            raise ValueError("source alias dictionary SHA-256 mismatch")
        temporary.replace(target)
        manifest[key] = {
            "uri": f"s3://oa-bolinas/{key}",
    ...
    for kind, key, group_column, chrom_column, allowed in [
        (
            ...
            "region_label",
            "source_chrom",
            {"cds", "utr3", "tss_region_and_utr5"},
        ),
        ("functional", extras[1], "source_arm", "chrom", {"ncrna", "enhancer"}),
    ]:
        counts = {}
        for batch in pq.ParquetFile(directory / "s3" / "oa-bolinas" / key).iter_batches(
            columns=[group_column, chrom_column], batch_size=8192
        ):
    ...
    (directory / "anchor-counts.json").write_text(
