...
def inputs(tmp_path: Path) -> tuple[Path, Path]:
    ...
    prepare_chain_requests(
        FIXTURE / "anchors.tsv", FIXTURE / "source.sizes", requests, bed
    )
    validate_chain_inputs(
        FIXTURE / "human_to_target.chain",
        FIXTURE / "source.sizes",
        FIXTURE / "target.sizes",
        assets(),
...
def project(tmp_path: Path) -> tuple[pl.DataFrame, pl.DataFrame, dict[str, object]]:
    write_chain_projections(
        ...
        tmp_path / "mapped.bed",
        tmp_path / "unmapped.bed",
        FIXTURE / "target.sizes",
        SPECIES,
        "Papio_anubis",
        tmp_path / "validated.json",
        tmp_path / "accepted.parquet",
        tmp_path / "rejected.parquet",
        tmp_path / "audit.json",
    )
    return (
...
def test_center_identity_and_strand_resizing(tmp_path: Path) -> None:
    ...
    assert rejected.select("query_name", "rejection_reason").rows() == [
        ("unmapped", "unmapped")
    ]
...
def test_partition_fails_closed(
    tmp_path: Path, bad_mapped: str, bad_unmapped: str
) -> None:
    inputs(tmp_path)
...
def test_unmapped_coordinates_are_checked(tmp_path: Path) -> None:
    ...
    (tmp_path / "mapped.bed").write_text("")
    (tmp_path / "unmapped.bed").write_text(
        bed.read_text().replace("227\t228", "226\t227")
    )
...
def test_checksums_and_direction_are_checked(tmp_path: Path) -> None:
    ...
    with pytest.raises(ValueError, match="SHA-256"):
        validate_chain_inputs(
            ...
            FIXTURE / "source.sizes",
            FIXTURE / "target.sizes",
            {**expected, "chain_sha256": "0" * 64},
            tmp_path / "audit.json",
        )
    # Pin the wrong dictionaries: hashes match, but the direction must fail.
    expected["source_sizes_sha256"], expected["target_sizes_sha256"] = (
        expected["target_sizes_sha256"],
        expected["source_sizes_sha256"],
    )
...
def test_request_checks_exact_source_dictionary(tmp_path: Path) -> None:
    ...
    with pytest.raises(ValueError, match="human bounds"):
        prepare_chain_requests(
            FIXTURE / "anchors.tsv", bad, tmp_path / "r.parquet", tmp_path / "r.bed"
        )
...
def test_empty_chain_fails_even_if_hash_matches(tmp_path: Path) -> None:
    ...
    with pytest.raises(ValueError, match="no headers"):
        validate_chain_inputs(
            ...
            FIXTURE / "source.sizes",
            FIXTURE / "target.sizes",
            {**assets(), "chain_sha256": file_sha256(bad)},
            tmp_path / "audit.json",
        )


def test_numeric_and_leading_zero_query_names_are_not_reinterpreted(
    tmp_path: Path,
) -> None:
    anchors = tmp_path / "numeric.tsv"
    anchors.write_text(
        "query_name\tsource_chrom\tsource_start\tsource_end\tregion_label\n"
        "001\tchr1\t100\t355\tcds\n"
        "1\tchr1\t300\t555\tenhancer\n"
        "2\tchr1\t600\t855\tncrna\n"
    )
    requests, bed = tmp_path / "requests.parquet", tmp_path / "input.bed"
    prepare_chain_requests(anchors, FIXTURE / "source.sizes", requests, bed)
    assert pl.read_parquet(requests)["query_name"].to_list() == ["001", "1", "2"]
    assert read_bed6(bed)["query_name"].to_list() == ["001", "1", "2"]
