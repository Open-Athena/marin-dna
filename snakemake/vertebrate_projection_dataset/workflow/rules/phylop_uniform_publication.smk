"""Split, validate, and publish the issue #517 strict phyloP-control datasets."""
...
assert SOURCE_PIPELINE_VERSION == "phylop-uniform-v1"
assert int(config["phylop_min_selected_bases"]) == 51
assert int(config["phylop_window_size"]) == 255
assert float(config["phylop_threshold"]) == 2.2162
assert SOURCE_ROWS > 0
assert all(rows > 0 for rows in SOURCE_ARM_ROWS.values())


rule phylop_dataset_splits:
    input:
        COMBINED_SEQUENCES,
    output:
...
rule phylop_prepare_train_jsonl_shards:
    input:
...
rule phylop_prepare_validation_jsonl_shards:
    input:
...
rule phylop_compress_publication_shard:
    input:
    ...
    run:
        write_dataset_card(
            ...
            anchor_provenance=(
                ...
                "with `hg38.phyloP447way >= 2.2162`. The six labels are the issue "
                "#232 v4 CDS, 3-prime UTR, protein-coding TSS/5-prime UTR, and "
                "ncRNA-exon assignments; issue #326 Arm A enhancer; and the "
                "exhaustive phyloP-constrained remainder background."
            ),
        )


rule phylop_hf_artifact_manifest:
    input:
    ...
    run:
        ...
        publication = validate_artifacts(
            ...
            f"{RESULTS}/datasets",
            output[0],
            config_path="config/phylop_uniform_publication.yaml",
            pipeline_commit=PIPELINE_COMMIT,
            config_sha256=PIPELINE_CONFIG_SHA256,
            source_artifacts={
                cohort: {
...
rule all_phylop_hf:
    """Upload and verify all six datasets after explicit training authorization."""
    input:
