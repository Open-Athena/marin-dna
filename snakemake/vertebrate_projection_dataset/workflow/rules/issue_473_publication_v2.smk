"""Additive local-staging publication rules for issue #473.

The original issue-473 publication rules intentionally remain unchanged. This
second rule set keeps cards and the validation manifest local while they are
consumed, then archives the validated manifest through Snakemake storage.
"""

from pathlib import Path

ISSUE_473_V2_PUBLICATION_ROOT = (
    f"results/{ISSUE_473_PIPELINE_VERSION}/{ISSUE_473_PUBLICATION_COMMIT}/"
    f"{ISSUE_473_PUBLICATION_CONFIG_SHA256}/issue_473_publication_v2"
)
ISSUE_473_V2_HF_RESULTS = f"{ISSUE_473_V2_PUBLICATION_ROOT}/hf"
ISSUE_473_V2_HF_MANIFEST = (
    f"{ISSUE_473_V2_PUBLICATION_ROOT}/validation/hf_publication_manifest.json"
)
ISSUE_473_V2_HF_MANIFEST_ARCHIVE = (
    f"{ISSUE_473_V2_PUBLICATION_ROOT}/validation/"
    "hf_publication_manifest.archived.json"
)


rule issue_473_v2_prepare_train_jsonl_shards:
    input:
    ...
    output:
        temp(
            local(
                expand(
                    f"{ISSUE_473_V2_HF_RESULTS}/{{{{dataset}}}}/data/train/"
                    "{shard}.jsonl",
...
rule issue_473_v2_prepare_validation_jsonl_shards:
    input:
    ...
    output:
        temp(
            local(
                expand(
                    f"{ISSUE_473_V2_HF_RESULTS}/{{{{dataset}}}}/data/validation/"
                    "{shard}.jsonl",
...
rule issue_473_v2_compress_publication_shard:
    input:
        local(f"{ISSUE_473_V2_HF_RESULTS}/{{dataset}}/data/{{split}}/" "{shard}.jsonl"),
    output:
        local(
            f"{ISSUE_473_V2_HF_RESULTS}/{{dataset}}/data/{{split}}/"
            "{shard}.jsonl.zst"
        ),
    wildcard_constraints:
        dataset=ISSUE_473_PUBLICATION_KEY_RE,
...
rule issue_473_v2_dataset_card:
    input:
    ...
    output:
        local(f"{ISSUE_473_V2_HF_RESULTS}/{{dataset}}/README.md"),
    wildcard_constraints:
        dataset=ISSUE_473_PUBLICATION_KEY_RE,
    resources:
        mem_mb=2000,
...
rule issue_473_v2_hf_artifact_manifest:
    input:
        ...
        cards=local(
            expand(
                f"{ISSUE_473_V2_HF_RESULTS}/{{dataset}}/README.md",
                dataset=ISSUE_473_PUBLICATION_KEYS,
            )
        ),
    output:
        local(ISSUE_473_V2_HF_MANIFEST),
    threads: 8
    resources:
        mem_mb=8000,
        final_large_scan=1,
...
rule issue_473_v2_archive_hf_artifact_manifest:
    input:
        local(ISSUE_473_V2_HF_MANIFEST),
    output:
        ISSUE_473_V2_HF_MANIFEST_ARCHIVE,
...
rule issue_473_v2_all_hf_files:
    input:
        local(ISSUE_473_V2_HF_MANIFEST),
        ISSUE_473_V2_HF_MANIFEST_ARCHIVE,
        local(
...
rule issue_473_v2_hf_upload_dataset:
    input:
        manifest=local(ISSUE_473_V2_HF_MANIFEST),
        train=lambda wc: [
            local(
                f"{ISSUE_473_V2_HF_RESULTS}/{wc.dataset}/data/train/"
                f"{shard}.jsonl.zst"
            )
        ...
        validation=lambda wc: [
            local(
                f"{ISSUE_473_V2_HF_RESULTS}/{wc.dataset}/data/validation/"
                f"{shard}.jsonl.zst"
            )
            for shard in ISSUE_473_VALIDATION_SHARDS
        ],
        card=local(f"{ISSUE_473_V2_HF_RESULTS}/{{dataset}}/README.md"),
    output:
        local(f"{ISSUE_473_V2_PUBLICATION_ROOT}/upload.done/{{dataset}}"),
    wildcard_constraints:
        dataset=ISSUE_473_PUBLICATION_KEY_RE,
    resources:
        hf_uploads=1,
...
rule issue_473_v2_all_hf:
    input:
        local(
            expand(
                f"{ISSUE_473_V2_PUBLICATION_ROOT}/upload.done/{{dataset}}",
                dataset=ISSUE_473_PUBLICATION_KEYS,
            )
        ),
