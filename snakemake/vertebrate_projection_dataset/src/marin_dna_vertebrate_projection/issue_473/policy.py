...
def policy_by_name(name: str) -> ProjectionPolicy:
    """Resolve one preregistered issue #473 policy by its artifact name."""
    if name == FULL_WINDOW_POLICY.name:
        return FULL_WINDOW_POLICY
    prefix = "center_"
    assert name.startswith(prefix), f"unknown projection policy: {name}"
    return centered_landmark_policy(int(name.removeprefix(prefix)))
