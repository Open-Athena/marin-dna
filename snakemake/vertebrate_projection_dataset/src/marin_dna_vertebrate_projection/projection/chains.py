...
from marin_dna_vertebrate_projection.pipeline_io import read_anchor_catalog
from marin_dna_vertebrate_projection.projection.center import read_projection_requests
from marin_dna_vertebrate_projection.projection.requests import build_projection_requests

ASSET_COLUMNS = {
    "alignment_name", "assembly", "source_assembly", "chain_origin",
    "chain", "chain_sha256", "source_sizes", "source_sizes_sha256",
    "target_sizes", "target_sizes_sha256",
}
BED_SCHEMA = {
    "t_chrom": pl.String, "t_start": pl.Int64, "t_end": pl.Int64,
    "query_name": pl.String, "score": pl.String, "t_strand": pl.String,
}


def read_chain_assets(path: str | Path, species_path: str | Path) -> dict[str, dict[str, str]]:
    ...
    with Path(path).open() as handle:
        ...
        for row in reader:
            ...
            if assemblies.get(name) != row["assembly"]:
                raise ValueError(f"chain target is not a version-matched selected assembly: {name}")
            if row["source_assembly"] != "hg38":
                raise ValueError("this adapter requires the pinned hg38 source coordinate system")
    ...
    sources = {(row["source_sizes"], row["source_sizes_sha256"]) for row in result.values()}
    if len(sources) != 1:
        raise ValueError("all targets must use the same pinned human chromosome dictionary")
    return result
    ...
    with Path(path).open() as handle:
        for line in handle:
            ...
            if len(fields) != 2 or fields[0] in sizes or int(fields[1]) <= 0:
                raise ValueError(f"invalid or duplicate chromosome size: {line.strip()}")
...
def validate_chain_inputs(
    ...
    """
    for key, path in (("chain", chain), ("source_sizes", source_sizes), ("target_sizes", target_sizes)):
    ...
    with opener(chain, "rt") as handle:
        for line in handle:
            ...
            _, score, t_name, t_size, t_strand, t_start, t_end, q_name, q_size, q_strand, q_start, q_end, chain_id = fields
            int(score)
            int(chain_id)
            if source.get(t_name) != int(t_size) or target.get(q_name) != int(q_size):
                raise ValueError(f"chain direction/assembly size mismatch: {t_name} -> {q_name}")
    ...
    Path(output).write_text(json.dumps({"assets": expected, "chain_headers": count}, indent=2) + "\n")
...
def prepare_chain_requests(
    ...
    if anchors["query_name"].str.contains(r"\s").any() or (anchors["query_name"].str.len_chars() == 0).any():
        raise ValueError("query names must be nonempty BED-safe unique identifiers")
    sizes = read_sizes(source_sizes_path)
    for chrom, end in anchors.group_by("source_chrom").agg(pl.col("source_end").max()).iter_rows():
    ...
    requests.select(
        "source_chrom", "projection_start", "projection_end", "query_name",
        pl.lit("0").alias("score"), pl.lit("+").alias("strand"),
...
def write_chain_projections(
    ...
    mapped_names, unmapped_names = set(mapped["query_name"]), set(unmapped["query_name"])
    ...
    expected_unmapped = requests.filter(pl.col("query_name").is_in(unmapped_names)).select(
        pl.col("source_chrom").alias("t_chrom"),
        pl.col("projection_start").alias("t_start"),
        pl.col("projection_end").alias("t_end"), "query_name",
        pl.lit("0").alias("score"), pl.lit("+").alias("t_strand"),
    )
    ...
    metadata = metadata.select(
        pl.col("scientific_name").alias("species"), "alignment_name", "assembly",
        "taxonomy_id", "family", "clade", "phylogenetic_rank",
        pl.col("backend").alias("alignment_source"),
    )
    validation = json.loads(Path(validation_path).read_text())
    assets = validation["assets"]
    if assets["alignment_name"] != alignment_name or assets["assembly"] != metadata["assembly"][0]:
    ...
    size_frame = pl.DataFrame({"t_chrom": list(sizes), "t_src_size": list(sizes.values())})
    joined = mapped.join(requests, on="query_name", validate="m:1").join(metadata, how="cross").join(size_frame, on="t_chrom", how="left")
    if joined["t_src_size"].null_count() or (joined["t_end"] > joined["t_src_size"]).any():
        raise ValueError("mapped center exceeds pinned target bounds")
    fragments = joined.with_row_index("_id").select(
        "query_name", "source_chrom", "source_start", "source_end", "region_label",
        pl.col("projection_start").alias("source_fragment_start"),
        pl.col("projection_end").alias("source_fragment_end"),
        "species", "alignment_name", "assembly", "taxonomy_id", "family", "clade",
        "phylogenetic_rank", "alignment_source", "t_chrom", "t_start", "t_end", "t_strand", "t_src_size",
    ...
    missing_rows = requests.filter(pl.col("query_name").is_in(unmapped_names)).join(metadata, how="cross").with_columns(
        pl.lit("unmapped").alias("rejection_reason"),
        pl.lit("liftOver reported no mapping; see raw unmapped BED for reason").alias("detail"),
        pl.lit(0, dtype=pl.Int64).alias("fragment_count"),
    ).select(list(rejected_schema)).cast(rejected_schema)
    ...
    if accepted.height + rejected.height != requests.height:
        raise ValueError("accepted/rejected output does not account for every input query")
    ...
    Path(audit_path).write_text(json.dumps({
        "projection_policy": "center_1", "coordinate_system": "0-based half-open",
        "projector": "UCSC liftOver", "options": ["-minMatch=0.95", "-multiple"],
        "assets": assets, "requests_sha256": file_sha256(requests_path),
        "species_manifest_sha256": file_sha256(species_path),
        "input_queries": requests.height, "mapped_rows": mapped.height,
        "mapped_queries": len(mapped_names), "unmapped_queries": len(unmapped_names),
        "accepted_queries": accepted.height, "rejected_queries": rejected.height,
        "rejections": dict(rejected.group_by("rejection_reason").len().iter_rows()),
    }, indent=2) + "\n")
