...
```bash
uv run --locked snakemake all_annotation_source_stats --cores 4
```
...
Ensure the genome-selection output is available at the path configured by this pipeline, then use the project-local environment:
...
generated by `marin_dna_training_dataset.hf_readme`
...
`tests/training_dataset/test_hf_readme.py`).
