"""Tests for ``marin_dna_evals.model.scoring`` (CLM-only).
...
here. After the migration, ``marin_dna_evals.model.scoring`` calls
...
def test_run_reflogprob_clm_fwd_and_rc_differ():
    ...
    ``run_inference`` produce distinct outputs. ``marin_dna_evals.model.runner``
...
def test_run_variant_score_bundle_rc_returns_both_strands(tmp_path):
    ...
    from marin_dna_evals.transforms import _get_special_token_counts, in_seq_var_pos
