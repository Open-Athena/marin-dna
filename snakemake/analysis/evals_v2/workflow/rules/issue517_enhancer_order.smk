...
from marin_dna_evals.issue517_train_only import load_order_development_frame


rule compute_scores_exp517_order:
    input:
        checkpoint="results/checkpoints/{model}",
    output:
        "results/scores/{model}/{dataset}.parquet",
    wildcard_constraints:
        model="exp517-phylop-uniform-enhancer-order-step-4999",
    ...
    run:
        ...
        ds = load_order_development_frame(
            params.hf_path, revision=params.hf_revision, split=config["split"]
        )
