...
Family = Literal["marin_dna", "conservation", "alphagenome", "gpn_star", "evo2"]
...
def _find_repo_root(start: Path) -> Path:
    for parent in (start, *start.parents):
        if (parent / "dashboard" / "models.yaml").is_file():
            return parent
    raise RuntimeError(f"could not find dashboard/models.yaml above {start}")
...
def _coerce(raw: dict) -> Model:
    ...
    if family == "marin_dna":
        assert checkpoint is not None, (
            f"family=marin_dna requires `checkpoint:` block (id={raw['id']!r})"
        )
        assert checkpoint.gcs or checkpoint.hf, (
            f"family=marin_dna requires checkpoint.gcs or checkpoint.hf (id={raw['id']!r})"
        )
    return Model(
        id=raw["id"],
        display=raw["display"],
