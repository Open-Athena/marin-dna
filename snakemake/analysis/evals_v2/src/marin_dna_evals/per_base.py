...
def compare_ll_gap_cache(
    ...
    cached: pd.DataFrame,
    *,
    per_window_atol: float = 5e-4,
    aggregate_mean_atol: float = 1e-6,
    ...
    for value in ("ll_sum_upper", "ll_sum_lower"):
        delta = joined[f"{value}_new"] - joined[f"{value}_cached"]
        max_abs = max(max_abs, float(np.abs(delta).max()))
        assert np.abs(delta).max() <= per_window_atol, (
            f"{value} per-window max abs diff {np.abs(delta).max():.3g} "
            f"> {per_window_atol}"
        )
        count = "n_upper" if value.endswith("upper") else "n_lower"
        mean_diff = float(delta.sum() / joined[f"{count}_new"].sum())
        mean_diffs[value] = mean_diff
    ...
    return {
        ...
        "ll_upper_mean_diff": mean_diffs["ll_sum_upper"],
        "ll_lower_mean_diff": mean_diffs["ll_sum_lower"],
