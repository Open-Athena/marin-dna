...
Genomic language models (gLMs) have been developed using a wide range of specialized architectures,[^glm-architecture] long-context methods,[^glm-tokenization] and genomics-specific priors.[^glm-biology][^glm-review] We take a different approach: keep the model close to a standard GPT architecture and focus on data curation, optimization, scaling, and data mixtures. The experiments below test how far that simpler, standardized recipe can go.

[^glm-review]: For a broader overview, see [“Genomic language models: opportunities and challenges”](https://doi.org/10.1016/j.tig.2024.11.013).
