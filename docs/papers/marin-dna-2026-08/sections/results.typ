...
m5.1's three-stage lineage consumed 166.01B tokens and 1.135e21 recorded FLOPs, compared with 9.3T tokens and an estimated 2.25e24 FLOPs for Evo 2 40B---a ratio of 1,982, reported as approximately 1,980×.
At their native context lengths on the same GH200, the optimized m5.1 configuration extrapolates to one million variants in about 41 minutes, compared with roughly 66 days for Evo 2 40B---an approximately 2,330× throughput advantage.#footnote[This benchmark measures pre-materialized steady-state scoring with forward- and reverse-complement passes and embeddings enabled. The m5.1 throughput coordinate uses delayed FP8 at a 256-token context, while its plotted AUPRC is the frozen BF16 result; the FP8 configuration passed the paired zero-shot quality gate reported in Methods. Evo 2 40B uses its original inference stack at an 8,192-token context. The Evo 2 rate is rounded at source, which limits the precision of the ratio. This measures as-deployed throughput rather than same-context or per-token efficiency. See #link("https://github.com/Open-Athena/marin-dna/issues/354")[issue \#354] for the full methodology and results.]
